...
trace <- read.table(
  ...
  sep = "",
  header = TRUE,
  colClasses = c("integer", "numeric", "character", "numeric", "numeric", "numeric", "character", "numeric", "numeric")
)

# 列名の設定
colnames(trace) <- c("run", "fit1", "node1", "x1_1", "x2_1", "fit2", "node2", "x1_2", "x2_2")

# ノードとエッジデータの準備
lnodes <- rbind(
  setNames(trace[c("node1", "fit1", "x1_1", "x2_1")], c("Node", "Fitness", "x1", "x2")),
  setNames(trace[c("node2", "fit2", "x1_2", "x2_2")], c("Node", "Fitness", "x1", "x2"))
)

ledges <- trace[c("node1", "node2")]

# ノードとエッジの集計
nodes <- ddply(lnodes, .(Node, Fitness, x1, x2), nrow)
colnames(nodes) <- c("Node", "Fitness", "x1", "x2", "Count")
...
lonlay <- cbind(V(LON)$x1, V(LON)$x2)
...
lonlay2 <- cbind(V(CMLON)$x1, V(CMLON)$x2)#layout_nicely(CMLON)#layout_with_kk
print(lonlay2)
