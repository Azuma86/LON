...
# -------------------------------------
cv_all  = df_all['CV'].values
max_cv  = cv_all.max()
if max_cv == 0:
    colors_lower = ['red'] * len(df_all)
else:
    norm_all = mcolors.Normalize(vmin=0, vmax=max_cv)
    cmap_b   = plt.colormaps['Blues']
    colors_lower = [
        'red' if c == 0 else cmap_b(norm_all(c))
        for c in cv_all
    ]
...
# 対角：全データのヒストグラム
g.map_diag(sns.histplot, kde=False, bins=10, color='lightgray', edgecolor='black')
